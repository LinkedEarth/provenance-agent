...
no model call happens). The software dispatch path runs fully offline with
fmt="bibtex" (local Citations/ only, no Gemini).
"""
...
def test_run_dispatches_to_chosen_tool(monkeypatch):
    monkeypatch.setattr(
        agent, "route",
        lambda request, notebook_path: [
            {"name": "cite_software",
             "args": {"notebook_path": SAMPLE, "libraries": "pyleoclim", "fmt": "bibtex"}}
        ],
    )
    out = agent.run("cite pyleoclim", SAMPLE)
    assert len(out) == 1
    assert out[0]["name"] == "cite_software"
    assert "pyleoclim" in out[0]["result"].lower()
